<template>
  <header class="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border safe-top transition-colors">
    <div class="max-w-[1440px] mx-auto px-3 sm:px-5 lg:px-8 h-[58px] sm:h-[60px] flex items-center justify-between gap-2 sm:gap-4">
      ...
      <nav class="hidden lg:flex items-center gap-0.5 lg:gap-1 bg-muted/50 p-1 rounded-xl border border-border flex-shrink min-w-0 overflow-x-auto no-scrollbar">
        <button
      ...
      <div class="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
        ...
        <div class="relative hidden sm:block w-48 md:w-64 lg:w-52 xl:w-64">
          ...
          <input
            ...
            class="w-full h-9 bg-muted/50 border border-border rounded-xl pl-8.5 pr-8 text-xs lg:text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/40 focus:bg-background transition"
          />
          <button aria-label="Clear search"
            v-if="searchQuery"
            @click="$emit('update:searchQuery', '')"
            class="absolute right-1.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition"
          >
            <X class="w-3.5 h-3.5" />
          </button>
        </div>

        <!-- User Account Menu: Display avatar only in navigation header -->
        <UserMenu placement="bottom-end" :show-label="false" />
      </div>
    </div>

    <!-- Mobile Search Bar (iOS Standard Height & Spacing) -->
    <div class="sm:hidden px-4 pt-0.5 pb-2">
